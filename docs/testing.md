...
| `make e2e-service` | RBAC isolation, immutable OCI source publication, pre-build target-contract failure, safe caller retry, verified output manifest digest, ephemeral logs, TTL cleanup, and host pull. | `examples/simple`, `.work/result-service.jsonl` |
