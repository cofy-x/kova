...
- 1–100 unique logical targets, each containing a tagged output image reference and an explicit `linux/amd64` or `linux/arm64` platform;
- bounded build options and an optional caller-scoped idempotency key.

The source zip contains top-level image directories with `Dockerfile` and `metadata.json` files. Each metadata object contains the same `target` and `platform` pair exposed by the public request.
...
Logical targets cannot end in `_nydus_v3`; Kova reserves that suffix for the concrete Nydus output so format expansion remains collision-free.
The controller inspects the fetched and digest-verified archive before invoking BuildKit and requires its normalized `(target, platform)` set to equal `spec.targets` exactly.
Missing, extra, platform-mismatched, invalid, or duplicate source targets produce `InvalidSource` or `InvalidTargets` without an image push.
...
Each successful output contains its format, image reference, verified manifest digest, immutable reference, and verified platform.
...
6. The runner dispatches every target to the worker pool declared for its platform and forces that platform in the BuildKit request.
7. The controller resolves each pushed digest, verifies the single-platform image configuration matches the request, and records the digest and platform with bounded parallelism.
...
flowchart LR
  ...
  amd64["linux/amd64 BuildKit pool"]
  arm64["linux/arm64 BuildKit pool"]
