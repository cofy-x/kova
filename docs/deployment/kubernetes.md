...
Use the [non-local deployment baseline](../../deploy/cloud-values.yaml) for a
remote cluster. Supply a separate values overlay for the target
...
`deploy/cloud-values.yaml` enables a conservative PDB by default and leaves HPA
disabled until the target cluster has the metrics signal needed to scale worker
Pods predictably.
...
for local kind this is usually `localhost:5002/kova:dev`, while cloud
deployments should use a registry available inside the cloud network.
