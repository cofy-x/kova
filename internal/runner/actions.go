...
func (c *Client) Scale(target string) (err error) {
	...
	replicas, err := strconv.Atoi(target)
	if err != nil || replicas < 0 {
		return fmt.Errorf("--target must be a non-negative integer")
	}
	kube, err := c.kubeClient()
	if err != nil {
		return err
	}
	if err := kube.ScaleDeployment(ctx, kovaWorkerNamespace, kovaWorkerDeployment, int32(replicas)); err != nil {
		return err
	}
